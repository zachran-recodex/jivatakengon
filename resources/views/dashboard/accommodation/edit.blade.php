        ...
        <div class="w-full mx-auto sm:px-6 lg:px-8">
            <div class="pb-4 flex justify-between">
                <x-dashboard-breadcrumb :links="[
                    ...
                    ['label' => 'Accommodation', 'url' => route('admin.accommodation.index')],
                    ['label' => $accommodation->title],
                    ['label' => 'Edit']
                ]" />

                <a href="{{ route('admin.accommodation.index') }}" class="border border-gray-200 rounded-lg bg-blue-500 hover:bg-blue-700 text-white font-bold py-3 px-5">
                    Back
                </a>
            </div>
                    ...
                    <form action="{{ route('admin.accommodation.update', $accommodation) }}" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="mb-4 flex items-center justify-between gap-4">
                            ...
                            <div class="w-full">
                                ...
                                <input type="text"
                                       ...
                                       id="subtitle"
                                       class="appearance-none border border-gray-200 rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"
                                       value="{{ old('subtitle', $accommodation->subtitle) }}"
                                       required>
                            </div>
                        </div>
                        ...
                        <div class="mb-4 flex items-center justify-between gap-4">
                            <div class="flex items-center justify-center w-full">
                                <label for="image" class="flex flex-col items-center justify-center w-full h-64 border-2 border-gray-200 border-dashed rounded-lg cursor-pointer bg-gray-50 hover:bg-gray-100">
                                    <div class="flex flex-col items-center justify-center pt-5 pb-6">
                                        ...
                                        <p class="text-xs text-gray-500 dark:text-gray-400">JPEG, PNG or JPG (MAX. 370x250)</p>
                                    </div>
                            ...
                            @if($accommodation->image)
                                <div class="w-full">
                                    <img src="{{ asset('storage/' . $accommodation->image) }}" alt="{{ $accommodation->title }}" class="w-full h-64 border object-cover rounded">
                                </div>
                            @endif
                        </div>
                        ...
                        <div class="mb-4">
                            ...
                            <select name="is_active"
                                    ...
                                    class="appearance-none border border-gray-200 rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"
                                    required>
                                <option value="1" {{ old('is_active', $accommodation->is_active) == 1 ? 'selected' : '' }}>Active</option>
                                <option value="0" {{ old('is_active', $accommodation->is_active) == 0 ? 'selected' : '' }}>Disable</option>
                            </select>
                        </div>
