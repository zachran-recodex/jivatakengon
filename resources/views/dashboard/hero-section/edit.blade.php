        ...
        <div class="w-full mx-auto sm:px-6 lg:px-8">
            ...
            <div class="mx-auto bg-gray-50 border border-gray-200 rounded-lg overflow-hidden">
                <div class="px-6 py-4">
                    ...
                    <form action="{{ route('admin.hero-section.update', $heroSection) }}" method="POST" enctype="multipart/form-data">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="mb-4 flex items-center justify-between gap-4">
                            ...
                            @if($heroSection->image)
                                <div class="w-full">
                                    <img src="{{ asset('storage/' . $heroSection->image) }}" alt="{{ $heroSection->title }}" class="w-full h-64 border object-cover rounded">
                                </div>
                            @endif
                        </div>
                        ...
                        <div class="mb-4">
                            ...
                            <select name="is_active"
                                    ...
                                    class="appearance-none border border-gray-200 rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"
                                    required>
                                <option value="1" {{ old('is_active', $heroSection->is_active) == 1 ? 'selected' : '' }}>Active</option>
                                <option value="0" {{ old('is_active', $heroSection->is_active) == 0 ? 'selected' : '' }}>Disable</option>
                            </select>
                        </div>
